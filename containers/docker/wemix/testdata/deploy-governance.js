...
var GovernanceDeployer = new function() {
    this.wallet = ["keystore/account1", "keystore/account2", "keystore/account3"]
    this.account = []
    this.from = null
    this.gas = 21000 * 1400
    ...
    this.nonce = function(index) {
        return this._nonce[index]++
    }
    ...
    this.deployContract = function(data) {
        ...
        var stx = offlineWalletSignTx(this.account[0].id, tx, eth.chainId())
    ...
    this.sendTx = function(to, value, data) {
        var tx = {from:this.from, to:to, gas:this.gas,
                  gasPrice:this.gasPrice, nonce:this.nonce(0)}
        if (value)
            tx.value = value
        if (data)
            tx.data = data
    ...
    this.sendStakingDeposit = function (from, nonce, to, data, stake) {
        this.log("staker addr = " + from.address)
        var tx = { from: from.address, to: to, gas: this.gas, gasPrice: this.gasPrice, nonce: nonce, value: "0" }
        tx.value = stake
        if (data) tx.data = data
        var stx = offlineWalletSignTx(from.id, tx, eth.chainId())

        return eth.sendRawTransaction(stx)
    }

    // bool deploy(string walletUrl, string password, string cfg)
    this.deploy = function(cfg, doInitOnce) {
        for (var i = 0; i < this.wallet.length; i++) {
            w = offlineWalletOpen(this.wallet[i], "1")
            if (!w || !w.id || !w.address) {
                throw "Offline wallet is not loaded"
            }
            this.account[i] = w
            this._nonce[i] = eth.getTransactionCount(this.account[i].address, 'pending')
        }
        this.from = this.account[0].address
        ...
        for (var i = 1; i < this.account.length; i++) {
            var d = tmpStakingImp.balanceOf.getData(this.account[i].address)
            txs[txs.length] = this.sendStakingDeposit(this.account[i], this.nonce(i), staking.address, tmpStakingImp.deposit.getData(), web3.toBigNumber(bootNode.stake).toString(10));
        }

        this.log("txs.length = " + txs.length)
        for(i=0;i<txs.length;i++){
            this.log("txs["+i+"] = " + txs[i])
        ...
        var ddd
        if (!doInitOnce) {
            ddd = tmpGovImp.init.getData(registry.address,
                    bootNode.stake, bootNode.name, bootNode.id,
                    bootNode.ip, bootNode.port)
        } else {
            this.log("initData = " + initData.nodes)
            ddd = tmpGovImp.initOnce.getData(registry.address, data.members[0].stake, initData.nodes)
        }
        this.log('data = ' + ddd)
        txs.length = 0
        txs[txs.length] = this.sendTx(gov.address, null, ddd)
