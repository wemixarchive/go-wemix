...
function init_gov ()
{
    ...
    exec ${GWEMIX} attach http://localhost:${PORT} --preload "$d/conf/WemixGovernance.js,$d/conf/deploy-governance.js" --exec 'GovernanceDeployer.deploy("'${CONFIG}'", '${INIT_ONCE}')'
}
...
function start ()
{
    ...
    RPCOPT="--http --http.addr 0.0.0.0 --mine"
...
function usage ()
{
	...
	init-gov <node> <config.json> <do-init-once>|
