...
// ===========================================================================
// V8: positive assertions for decideWorkKeyRecovery
// ===========================================================================
//
// These tests drive the pure decision body of maybeRecoverWorkKey directly,
// so the chain reader and etcd are replaced by stub closures. They lock in:
//
//   - the hash-reachable + height-match short-circuit (healthy state)
//   - the hash-reachable + height-mismatch path (Hole-A: inconsistent tuple
//     is treated as poison and corrected)
//   - the hash-unreachable + strictly-behind path (the original poison
//     auto-recovery case)
//   - the hash-unreachable + at-or-above-local-head guard (Hole-C: a
//     slow-syncing or minority-fork node must not overwrite a healthy
//     wemixWorkKey with its lagging local snapshot)
//   - input-safety short-circuits (empty value, malformed JSON, zero hash,
//     missing local head)
//
// Without these, future refactors of maybeRecoverWorkKey could quietly drop
// the corrective PUT or the regression guard and the existing negative
// regression tests (V7) would still pass.

func mustMarshalWork(t *testing.T, height int64, hash common.Hash) string {
	t.Helper()
	b, err := json.Marshal(&wemixWork{Height: height, Hash: hash})
	if err != nil {
		t.Fatalf("marshal wemixWork: %v", err)
	}
	return string(b)
}

func TestDecideWorkKeyRecovery_HealthyHashAndHeight_Skip(t *testing.T) {
	hashA := common.HexToHash("0xa1a1")
	workValue := mustMarshalWork(t, 1000, hashA)

	lookupByHash := func(h common.Hash) (*big.Int, bool) {
		if h == hashA {
			return big.NewInt(1000), true
		}
		return nil, false
	}
	lookupLocalHead := func() (*big.Int, common.Hash, bool) {
		return big.NewInt(1000), hashA, true
	}

	got, doCorrect := decideWorkKeyRecovery(workValue, lookupByHash, lookupLocalHead)

	reportInvariants(t, []invariant{
		{id: "I1", desc: "doCorrect=false when hash reachable and height matches", ok: !doCorrect},
		{id: "I2", desc: "no corrected payload returned", ok: got == ""},
	})
}

func TestDecideWorkKeyRecovery_HashReachableHeightMismatch_Corrects(t *testing.T) {
	// Hole-A: an inconsistent (height, hash) tuple — the hash lives at a
	// different local height than recorded — must be treated as poison.
	hashA := common.HexToHash("0xa1a1")
	hashLocal := common.HexToHash("0xb2b2")
	workValue := mustMarshalWork(t, 990, hashA) // recorded height 990

	lookupByHash := func(h common.Hash) (*big.Int, bool) {
		if h == hashA {
			return big.NewInt(800), true // but locally hashA is at height 800
		}
		return nil, false
	}
	lookupLocalHead := func() (*big.Int, common.Hash, bool) {
		return big.NewInt(1000), hashLocal, true
	}

	got, doCorrect := decideWorkKeyRecovery(workValue, lookupByHash, lookupLocalHead)
	expected := mustMarshalWork(t, 1000, hashLocal)

	reportInvariants(t, []invariant{
		{id: "I1", desc: "doCorrect=true for height-mismatch poison", ok: doCorrect},
		{id: "I2", desc: "corrected payload equals localHead serialization", ok: got == expected},
	})
}

func TestDecideWorkKeyRecovery_HashUnreachableBelowLocal_Corrects(t *testing.T) {
	// Original poison case the patch was written for: an unreachable hash
	// whose recorded height sits below our local head — we are confident the
	// recorded value is stale, recovery proceeds.
	staleHash := common.HexToHash("0xdead")
	hashLocal := common.HexToHash("0xb2b2")
	workValue := mustMarshalWork(t, 990, staleHash)

	lookupByHash := func(common.Hash) (*big.Int, bool) { return nil, false }
	lookupLocalHead := func() (*big.Int, common.Hash, bool) {
		return big.NewInt(1000), hashLocal, true
	}
	...
	reportInvariants(t, []invariant{
		{id: "I1", desc: "doCorrect=true for unreachable hash strictly behind local head", ok: doCorrect},
		{id: "I2", desc: "corrected payload equals localHead serialization", ok: got == expected},
	})
}

func TestDecideWorkKeyRecovery_HashUnreachableAtLocalHeight_Skip(t *testing.T) {
	// Hole-C, fork-at-tip flavor: recorded work points to canonical hash at
	// the same height as our local head, but our local head is on a fork. We
	// must NOT overwrite the canonical value with our minority-fork hash.
	canonicalHash := common.HexToHash("0xcafe")
	forkHash := common.HexToHash("0xbeef")
	workValue := mustMarshalWork(t, 1000, canonicalHash)

	lookupByHash := func(common.Hash) (*big.Int, bool) { return nil, false }
	lookupLocalHead := func() (*big.Int, common.Hash, bool) {
		return big.NewInt(1000), forkHash, true
	}

	got, doCorrect := decideWorkKeyRecovery(workValue, lookupByHash, lookupLocalHead)

	reportInvariants(t, []invariant{
		{id: "I1", desc: "doCorrect=false when work.Height == localHead.Number (possible fork)", ok: !doCorrect},
		{id: "I2", desc: "no corrected payload returned", ok: got == ""},
	})
}

func TestDecideWorkKeyRecovery_HashUnreachableAboveLocal_Skip(t *testing.T) {
	// Hole-C, slow-sync flavor: recorded work is ahead of our local head.
	// This is the legitimate "the network has moved on, we are behind" case
	// for a slow-syncing governance partner. The pre-patch behavior wrote
	// our lagging head into etcd, breaking mining cluster-wide. Recovery
	// must skip.
	networkHash := common.HexToHash("0xcafe")
	slowLocalHash := common.HexToHash("0x9999")
	workValue := mustMarshalWork(t, 1000, networkHash)

	lookupByHash := func(common.Hash) (*big.Int, bool) { return nil, false }
	lookupLocalHead := func() (*big.Int, common.Hash, bool) {
		return big.NewInt(990), slowLocalHash, true
	}

	got, doCorrect := decideWorkKeyRecovery(workValue, lookupByHash, lookupLocalHead)

	reportInvariants(t, []invariant{
		{id: "I1", desc: "doCorrect=false when work.Height > localHead.Number (slow sync)", ok: !doCorrect},
		{id: "I2", desc: "no corrected payload returned", ok: got == ""},
	})
}

func TestDecideWorkKeyRecovery_LocalHeadUnavailable_Skip(t *testing.T) {
	staleHash := common.HexToHash("0xdead")
	workValue := mustMarshalWork(t, 990, staleHash)
	...
	reportInvariants(t, []invariant{
		{id: "I1", desc: "doCorrect=false when local head lookup fails", ok: !doCorrect},
		{id: "I2", desc: "no corrected payload returned", ok: got == ""},
	})
}

func TestDecideWorkKeyRecovery_BadInputs_Skip(t *testing.T) {
	lookupByHash := func(common.Hash) (*big.Int, bool) { return nil, false }
	lookupLocalHead := func() (*big.Int, common.Hash, bool) {
		return big.NewInt(1000), common.HexToHash("0xb2b2"), true
	}
	...
	cases := []struct {
		...
		{"zero-hash", mustMarshalWork(t, 1000, common.Hash{})},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, doCorrect := decideWorkKeyRecovery(tc.workValue, lookupByHash, lookupLocalHead)
			if doCorrect || got != "" {
				t.Fatalf("expected skip for %s input, got doCorrect=%v value=%q", tc.name, doCorrect, got)
			}
		})
	}
}
