...
func TestGov(t *testing.T) {
	t.Run("Staker is voter", func(t *testing.T) {
		...
		t.Run("can addProposal to add member", func(t *testing.T) {
			...
			// staking first
			_, receipt := gov.client.SendTransaction(t, gov.client.Owner, govMem1, bn.Add(LOCK_AMOUNT, bn.ToWei18(1)), []byte{})
			...
			require.Equal(t, bn.New(1), length)
			...
			require.Equal(t, bn.New(2), gov.Gov.Call1(t, "ballotLength").(*big.Int))
		})
		t.Run("cannot addProposal to remove non-member", func(t *testing.T) {
			gov := DeployGovernance(t)
			govMem1 := sim.AliasToAddress["govMem1"]
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToRemoveMember", govMem1, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int)),
				"Non-member",
			)
		})
		t.Run("cannot addProposal to remove a sole member", func(t *testing.T) {
			gov := DeployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToRemoveMember", gov.client.Owner, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int)),
				"Cannot remove a sole member",
			)
		})
		t.Run("can addProposal to change member's other addresses self without voting", func(t *testing.T) {
			gov := DeployGovernance(t)
			oldMember := gov.Gov.Call1(t, "getMember", bn.New(1)).(common.Address)
			require.Equal(t, gov.client.Owner, oldMember)
			oldVoter := gov.Gov.Call1(t, "getVoter", bn.New(1)).(common.Address)
			require.Equal(t, gov.client.Owner, oldVoter)
			oldReward := gov.Gov.Call1(t, "getReward", bn.New(1)).(common.Address)
		...
		t.Run("can addProposal to change member's other addresses self without voting twice about node name", func(t *testing.T) {
			...
			newMember := gov.Gov.Call1(t, "getMember", bn.New(1)).(common.Address)
			require.Equal(t, gov.client.Owner, newMember)
			newVoter := gov.Gov.Call1(t, "getVoter", bn.New(1)).(common.Address)
			require.Equal(t, gov.client.Owner, newVoter)
			newReward := gov.Gov.Call1(t, "getReward", bn.New(1)).(common.Address)
			...
			length = gov.Gov.Call1(t, "ballotLength").(*big.Int)
			require.Equal(t, bn.New(2), length)
		...
		t.Run("can addProposal to change governance", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "addProposalToChangeGov", newGovImp.Address, []byte("memo"), bn.New(86400))
			length := gov.Gov.Call1(t, "ballotLength").(*big.Int)
			require.Equal(t, bn.New(1), length)
		})
		t.Run("can not addProposal to change governance using EOA", func(t *testing.T) {
			gov := DeployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToChangeGov", sim.GetOrNewEOA("govMem1"), []byte("memo"), bn.New(86400)),
		...
		t.Run("can not addProposal to change governance using non-UUPS", func(t *testing.T) {
			gov := DeployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToChangeGov", gov.Registry.Address, []byte("memo"), bn.New(86400)),
		...
		t.Run("cannot addProposal to change governance with same address", func(t *testing.T) {
			gov := DeployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToChangeGov", gov.GovImp.Address, []byte("memo"), bn.New(86400)),
		...
		t.Run("cannot addProposal to change governance with zero address", func(t *testing.T) {
			gov := DeployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToChangeGov", common.Address{}, []byte("memo"), bn.New(86400)),
				"Implementation cannot be zero",
			)
		})
		t.Run("can addProposal to change environment", func(t *testing.T) {
			gov := DeployGovernance(t)
			gov.Gov.ExecuteOk(t, gov.client.Owner, "addProposalToChangeEnv", sim.StringToBytes32("key"), EnvTypes.Bytes32, []byte("value"), []byte("memo"), bn.New(86400))
		...
		t.Run("cannot addProposal to change environment with wrong type", func(t *testing.T) {
			gov := DeployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "addProposalToChangeEnv", sim.StringToBytes32("key"), EnvTypes.Invalid, []byte("value"), []byte("memo"), bn.New(86400)),
				"Invalid type",
			)
		})
		t.Run("can vote approval to add member", func(t *testing.T) {
			...
			govMem1 := sim.GetOrNewEOA("govMem1")
			_, receipt := gov.client.SendTransaction(t, gov.client.Owner, govMem1, bn.Mul(LOCK_AMOUNT, 2), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
			gov.Staking.ExecuteWithETHOk(t, govMem1, LOCK_AMOUNT, "deposit")
			node1 := nodeInfo{
				name:  []byte("name1"),
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "vote", bn.New(1), true)
			length := gov.Gov.Call1(t, "voteLength").(*big.Int)
			require.Equal(t, bn.New(1), length)
			inVoting := gov.Gov.Call1(t, "getBallotInVoting").(*big.Int)
			require.True(t, inVoting.Sign() == 0)
			state := gov.BallotStorage.Call(t, "getBallotState", bn.New(1))
			...
			require.Equal(t, bn.New(2), memberLen)
		...
		t.Run("cannot vote approval to add member with insufficient staking", func(t *testing.T) {
			...
			_, receipt := gov.client.SendTransaction(t, gov.client.Owner, govMem1, bn.Mul(LOCK_AMOUNT, 2), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
			gov.Staking.ExecuteWithETHOk(t, govMem1, bn.Div(LOCK_AMOUNT, 2), "deposit")
			node1 := nodeInfo{
				name:  []byte("name1"),
				enode: hexutil.MustDecode("0x777777777711c39f35f516fa664deaaaa13e85b2f7493f37f6144d86991ec012937307647bd3b9a82abe2974e1407241d54947bbb39763a4cac9f77166ad92a0"),
				ip:    []byte("127.0.0.2"),
			...
			require.True(t, state[2].(bool))
			memberLen := gov.Gov.Call1(t, "getMemberLength").(*big.Int)
			require.Equal(t, bn.New(1), memberLen)
		})
		t.Run("can vote disapproval to deny adding member", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "vote", bn.New(1), false)
			length := gov.Gov.Call1(t, "voteLength").(*big.Int)
			require.Equal(t, bn.New(1), length)
			inVoting := gov.Gov.Call1(t, "getBallotInVoting").(*big.Int)
			require.True(t, inVoting.Sign() == 0)
		...
		t.Run("can vote approval to change member totally", func(t *testing.T) {
			...
			_, receipt = gov.client.SendTransaction(t, gov.client.Owner, govMem2, bn.Mul(LOCK_AMOUNT, 2), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
			gov.Staking.ExecuteWithETHOk(t, govMem2, LOCK_AMOUNT, "deposit")

			preDeployerAvail := gov.Staking.Call1(t, "availableBalanceOf", govMem1)
			preGovmem1Avail := gov.Staking.Call1(t, "availableBalanceOf", govMem2)
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "vote", bn.New(2), true)
			gov.Gov.ExecuteOk(t, govMem1, "vote", bn.New(2), true)
			length = gov.Gov.Call1(t, "voteLength").(*big.Int)
			require.Equal(t, bn.New(3), length)
			inVoting = gov.Gov.Call1(t, "getBallotInVoting").(*big.Int)
			require.True(t, inVoting.Sign() == 0)
			state = gov.BallotStorage.Call(t, "getBallotState", bn.New(2))
			require.Equal(t, BallotStates.Accepted, state[1].(*big.Int))
			require.True(t, state[2].(bool))

			memberLen := gov.Gov.Call1(t, "getMemberLength").(*big.Int)
			require.Equal(t, bn.New(2), memberLen)
			memberAddr := gov.Gov.Call1(t, "getMember", bn.New(2)).(common.Address)
			require.Equal(t, govMem2, memberAddr)
			getNode := gov.Gov.Call(t, "getNode", bn.New(2))
			name, enode, ip, port := getNode[0].([]byte), getNode[1].([]byte), getNode[2].([]byte), getNode[3].(*big.Int)
			...
			require.Equal(t, bn.New(2), nodeIdxFromGovMem1)

			postDeployerAvail := gov.Staking.Call1(t, "availableBalanceOf", govMem1)
			postGovmem1Avail := gov.Staking.Call1(t, "availableBalanceOf", govMem2)
			require.Equal(t, LOCK_AMOUNT, bn.Sub(postDeployerAvail, preDeployerAvail))
			require.Equal(t, LOCK_AMOUNT, bn.Sub(preGovmem1Avail, postGovmem1Avail))
		})
		t.Run("can vote approval to change enode only without voting", func(t *testing.T) {
			...
			memberLen := gov.Gov.Call1(t, ("getMemberLength")).(*big.Int)
			require.Equal(t, bn.New(1), memberLen)
			memberAddr := gov.Gov.Call1(t, "getMember", bn.New(1))
			require.Equal(t, gov.client.Owner, memberAddr)
			getNode := gov.Gov.Call(t, "getNode", bn.New(1))
			name, enode, ip, port := getNode[0].([]byte), getNode[1].([]byte), getNode[2].([]byte), getNode[3].(*big.Int)
			require.Equal(t, node1.name, name)
			require.Equal(t, node1.enode, enode)
			require.Equal(t, node1.ip, ip)
		...
		t.Run("cannot vote approval to change member with insufficient staking", func(t *testing.T) {
			...
			_, receipt := gov.client.SendTransaction(t, gov.client.Owner, govMem1, bn.Mul(LOCK_AMOUNT, 2), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
			gov.Staking.ExecuteWithETHOk(t, govMem1, bn.Sub(LOCK_AMOUNT, 1000000000), "deposit")
		...
		t.Run("can vote approval to change environment", func(t *testing.T) {
			...
			require.NotEqual(t, bn.New(100), blocksPer)
			...
			blocksPer = gov.EnvStorage.Call1(t, "getBlocksPer").(*big.Int)
			require.Equal(t, bn.New(100), blocksPer)
		})
		t.Run("cannot vote for a ballot already done", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "vote", bn.New(1), true)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "vote", bn.New(1), true),
				"Expired",
			)
		})
		t.Run("cannot add proposal durring period time", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "setProposalTimePeriod", bn.New(60))
			sim.ExpectedRevert(t,
		...
		t.Run("cannot addProposal to add member which is already reward", func(t *testing.T) {
			...
			require.Equal(t, bn.New(1), ballotLength)
			...
			ballotLength = gov.Gov.Call1(t, "ballotLength").(*big.Int)
			require.Equal(t, bn.New(2), ballotLength)
	...
	t.Run("Staker is not a voter", func(t *testing.T) {
		deployGovernance := func(t *testing.T) (gov *Governance, voter common.Address) {
			...
			_, receipt := gov.client.SendTransaction(t, gov.client.Owner, voter, bn.Div(balance, 2), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
		...
		t.Run("cannot addProposal to remove non-member", func(t *testing.T) {
			...
			govMem1 := sim.AliasToAddress["govMem1"]
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToRemoveMember", govMem1, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int)),
				"Non-member",
			)
		})
		t.Run("cannot addProposal to remove a sole member", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToRemoveMember", gov.client.Owner, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int)),
				"Cannot remove a sole member",
			)
		})
		t.Run("can addProposal to change member's other addresses self without voting", func(t *testing.T) {
			...
			length := gov.Gov.Call1(t, "ballotLength").(*big.Int)
			require.Equal(t, bn.New(2), length)
			gov.Gov.ExecuteOk(t, voter, "vote", bn.New(2), true)
		...
		t.Run("can addProposal to change governance", func(t *testing.T) {
			...
			newGovImp := compiled.Copy(new(Governance)).GovImp
			newGovImp.Deploy(t, gov.client)
			gov.Gov.ExecuteOk(t, voter, "addProposalToChangeGov", newGovImp.Address, []byte("memo"), bn.New(86400))
			length := gov.Gov.Call1(t, "ballotLength").(*big.Int)
			require.Equal(t, bn.New(2), length)
		})
		t.Run("can not addProposal to change governance using EOA", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToChangeGov", sim.GetOrNewEOA("govMem1"), []byte("memo"), bn.New(86400)),
				"", "function call to a non-contract account", // TODO check
			)
		})
		t.Run("can not addProposal to change governance using non-UUPS", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToChangeGov", gov.Registry.Address, []byte("memo"), bn.New(86400)),
				"ERC1967Upgrade: new implementation is not UUPS",
			)
		})
		t.Run("cannot addProposal to change governance with same address", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToChangeGov", gov.GovImp.Address, []byte("memo"), bn.New(86400)),
				"Same contract address",
			)
		})
		t.Run("cannot addProposal to change governance with zero address", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToChangeGov", common.Address{}, []byte("memo"), bn.New(86400)),
				"Implementation cannot be zero",
			)
		})
		t.Run("can addProposal to change environment", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			gov.Gov.ExecuteOk(t, voter, "addProposalToChangeEnv", sim.StringToBytes32("key"), EnvTypes.Bytes32, []byte("value"), []byte("memo"), bn.New(86400))
			length := gov.Gov.Call1(t, "ballotLength")
			require.Equal(t, bn.New(2), length)
		})
		t.Run("cannot addProposal to change environment with wrong type", func(t *testing.T) {
			gov, voter := deployGovernance(t)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "addProposalToChangeEnv", sim.StringToBytes32("key"), EnvTypes.Invalid, []byte("value"), []byte("memo"), bn.New(86400)),
				"Invalid type",
			)
		})
		t.Run("can vote approval to add member", func(t *testing.T) {
		...
		t.Run("can vote disapproval to deny adding member", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, voter, "vote", bn.New(2), false)
			length := gov.Gov.Call1(t, "voteLength").(*big.Int)
			require.Equal(t, bn.New(1), length)
			inVoting := gov.Gov.Call1(t, "getBallotInVoting").(*big.Int)
			require.True(t, inVoting.Sign() == 0)
		...
		t.Run("can vote approval to change member totally", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, voter, "vote", bn.New(3), true)
			gov.Gov.ExecuteOk(t, govMem1, "vote", bn.New(3), true)
			length = gov.Gov.Call1(t, "voteLength").(*big.Int)
			require.Equal(t, bn.New(3), length)
			inVoting = gov.Gov.Call1(t, "getBallotInVoting").(*big.Int)
			require.True(t, inVoting.Sign() == 0)
			state = gov.BallotStorage.Call(t, "getBallotState", bn.New(3))
			require.Equal(t, BallotStates.Accepted, state[1].(*big.Int))
			require.True(t, state[2].(bool))
		...
		t.Run("cannot vote for a ballot already done", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, voter, "vote", bn.New(2), true)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, voter, "vote", bn.New(2), true),
				"Expired",
			)
		})
		...
		t.Run("cannot vote for a ballot already voter done", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, voter, "vote", bn.New(2), true)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, gov.client.Owner, "vote", bn.New(2), true),
				"Expired",
			)
		})
		t.Run("cannot add proposal durring period time", func(t *testing.T) {
	...
	t.Run("Two Member", func(t *testing.T) {
		deployGovernance := func(t *testing.T) (gov *Governance, govMem1 common.Address) {
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "vote", bn.New(1), true)
			require.Equal(t, bn.New(2), gov.Gov.Call1(t, "getMemberLength").(*big.Int))
		...
		t.Run("cannot vote with changed voter address", func(t *testing.T) {
			...
			gov.client.SendTransaction(t, gov.client.Owner, voter, bn.ToWei18(1), []byte{})
			gov.client.SendTransaction(t, gov.client.Owner, voter1, bn.ToWei18(1), []byte{})
		...
		t.Run("can addProposal to remove member", func(t *testing.T) {
			gov, govMem1 := deployGovernance(t)
			gov.Gov.ExecuteOk(t, gov.client.Owner, "addProposalToRemoveMember", govMem1, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int))
			length := gov.Gov.Call1(t, "ballotLength").(*big.Int)
			require.Equal(t, bn.New(2), length)
		})
		...
		t.Run("can addProposal to add member where info is the removed member's", func(t *testing.T) {
			...
			govMem2 := sim.GetOrNewEOA("govMem2")
			_, receipt := gov.client.SendTransaction(t, gov.client.Owner, govMem2, bn.Add(LOCK_AMOUNT, bn.ToWei18(1)), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
			gov.Staking.ExecuteWithETHOk(t, govMem2, LOCK_AMOUNT, "deposit")
		...
		t.Run("can vote to remove first member", func(t *testing.T) {
			...
			gov.Gov.ExecuteOk(t, gov.client.Owner, "addProposalToRemoveMember", gov.client.Owner, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int))
			...
			nodeLen := gov.Gov.Call1(t, "getNodeLength").(*big.Int)
			require.Equal(t, bn.New(1), nodeLen)
			...
			require.Equal(t, bn.New(1), nodeIdx2)

			postAvail := gov.Staking.Call1(t, "availableBalanceOf", gov.client.Owner).(*big.Int)
			require.Equal(t, LOCK_AMOUNT, bn.Sub(postAvail, preAvail))
		})
		...
		t.Run("cannot vote simultaneously", func(t *testing.T) {
			...
			_, receipt = gov.client.SendTransaction(t, gov.client.Owner, govMem3, bn.Add(LOCK_AMOUNT, bn.ToWei18(1)), []byte{})
			require.Equal(t, types.ReceiptStatusSuccessful, receipt.Status)
			...
			length := gov.Gov.Call1(t, "ballotLength").(*big.Int)
			gov.Gov.ExecuteOk(t, gov.client.Owner, "vote", bn.Sub(length, 1), true)
			voting := gov.Gov.Call1(t, "getBallotInVoting")
			require.Equal(t, bn.Sub(length, 1), voting)
			sim.ExpectedRevert(t,
		...
		t.Run("cannot vote approval when the voting is ended", func(t *testing.T) {
			...
			gov.client.AdjustTime(t, time.Second*time.Duration(bn.Mul(delay_time, 2000).Int64()))
			sim.ExpectedRevert(t,
		...
		t.Run("Non member cannot end voting", func(t *testing.T) {
			...
			gov.client.SendTransaction(t, gov.client.Owner, govMem3, bn.ToWei18(1), []byte{})
			sim.ExpectedRevert(t,
		...
		t.Run("reject proposal without voting about changing voter address if voter is already registered", func(t *testing.T) {
			...
			memberLen := gov.Gov.Call1(t, "getMemberLength").(*big.Int)
			require.Equal(t, bn.New(2), memberLen)
			memberAddr := gov.Gov.Call1(t, "getMember", bn.New(1)).(common.Address)
			require.Equal(t, gov.client.Owner, memberAddr)
			voterAddr := gov.Gov.Call1(t, "getVoter", bn.New(1)).(common.Address)
			require.Equal(t, gov.client.Owner, voterAddr)
			rewardAddr := gov.Gov.Call1(t, "getReward", bn.New(1)).(common.Address)
	...
	t.Run("Others", func(t *testing.T) {
		...
		t.Run("cannot addProposal", func(t *testing.T) {
			gov := DeployGovernance(t)
			govMem1, govMem2 := sim.GetOrNewEOA("govMem1"), sim.GetOrNewEOA("govMem2")
			gov.client.SendTransaction(t, gov.client.Owner, govMem1, bn.ToWei18(1), []byte{})
			gov.client.SendTransaction(t, gov.client.Owner, govMem2, bn.ToWei18(1), []byte{})
			...
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, govMem1, "addProposalToRemoveMember", govMem1, LOCK_AMOUNT, []byte("memo1"), bn.New(86400), LOCK_AMOUNT, new(big.Int)),
				"No Permission",
			)
			...
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, govMem1, "addProposalToChangeGov", govMem1, []byte("memo"), bn.New(86400)),
				"No Permission", // TODO check
			)
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, govMem1, "addProposalToChangeEnv", sim.StringToBytes32("key"), EnvTypes.Bytes32, []byte("value"), []byte("memo"), bn.New(86400)),
				"No Permission",
			)
		})
		t.Run("cannot vote", func(t *testing.T) {
			...
			govMem1, govMem2 := sim.GetOrNewEOA("govMem1"), sim.GetOrNewEOA("govMem2")
			gov.client.SendTransaction(t, gov.client.Owner, govMem1, bn.ToWei18(1), []byte{})
			gov.client.SendTransaction(t, gov.client.Owner, govMem2, bn.Add(LOCK_AMOUNT, bn.ToWei18(1)), []byte{})
			gov.Staking.ExecuteWithETHOk(t, govMem2, LOCK_AMOUNT, "deposit")
			...
			sim.ExpectedRevert(t,
				gov.Gov.ExecuteFail(t, govMem1, "vote", bn.New(1), true),
				"No Permission",
			)
		})
	})
